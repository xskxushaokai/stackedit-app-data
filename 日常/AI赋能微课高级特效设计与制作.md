...
## 二. 案例1. 简单的课堂点名系统
课堂上，教师常需要通过点名提问的方式来及时了解学生的知识掌握情况。AI工具的出现之前，使得教师根据自己的课堂需求来定制点名系统成为可能。以下展示了使用豆包应用生成平台AI工具定制课堂点名系统的过程。
...
接下来，我们将通过一个功能更加完善的课堂测试系统来演示如何利用AgentI实现完整的前后端教学应用开发。Agent是在大模型的基础上，进一步集成了各类工具调用能力，由一个或多个大模型进行任务规划和协作，是更高级的AI自动化工具。

工欲善其事必先利其器，由于前后端共同开发涉及到的代码量和文件数量略有增加，且涉及到接口调用和通信，所以我们不再使用网页版的豆包应用生成平台，而是通过在本地安装使用专业的代码编辑器来完成应用开发。目前国内使用较多的集成了AgentI功能的编辑器主要有TRAE、VS Code、ClaudeCode、Cursor 等，我们使用TRAE来进行教学应用开发。虽然我们使用了“专业的代码编辑器”，但不用担心，我们仍然是通过与AI协作来完成应用的开发，在与AI协作的过程中，我们充当的角色是：应用的设计者、测试者和使用者，AI充当的角色是：开发者和测试工程师，代码编写和修改的任务仍然是交给AI来完成。

在本地安装TRAE与AI进行协作开发的好处是用户可以赋予AI文件操作和执行权限，从而实现更便捷的AI自动化操作。

通过官网下载（https://www.trae.cn/）下载安装包。
...
在右侧的智能体AI协作窗口中，TRAE中内置了四种AI智能体，分别是“chat”、“builder”、“Builder with MCP”、“SOLO Coder”，对于常规开发任务，选择“builder”即可，通过右下角的下拉箭头可以选择要使用的大模型，若读者对各个大模型不够了解，使用“Auto Mode”即可。
...
```
此时，我们再次按照AI给出的方法，点击Bash窗口右侧的运行按钮直接运行命令“node server.js”，运行后在TRAE界面的中间下方终端窗口中出现以下提示信息，表明HTTP服务已经成功启动。
```
数据初始化成功
服务器运行在 http://localhost:3000
默认教师账号: admin / admin123
...
复制HTTP服务的地址“http://localhost:3000”并将其粘贴到浏览器地址栏，即可成功打开课堂测试系统的首页，我们首先点击“教师登录”，输入用户名“admin”和密码“admin123”，点击登录后直接进入教师管理后台，可以看到“统计概览”、“学生管理”、“题目管理”、“测试设置”四个模块，此时的学生数量和题目数量均为0，我们先添加学生和测试题目
...
```
**注意：接下来我们要对后端代码进行修改，后端代码发生改变后，需要先停止当前的服务，再次重新启动服务更改才会生效。若同时启动两个相同端口号的HTTP服务，则会产生端口号冲突的问题导致服务无法启动。所以在向AI发起提问前，需要先停止当前正在运行的服务，在TRAE中间下方的终端界面中点击“终止”按钮或使用快捷键“CTRL+C”即可停止当前正在运行的服务。**
...
4. 局域网部署
若要让学生通过教室或者校园局域网内的其他设备访问我们的课堂测试系统，则需要将教师的个人电脑作为临时服务器，运行课堂测试系统的后端服务，并开启局域网公开访问接口和计算机防火墙等。同样，我们通过询问AI来得到具体的操作方法。提示词如下：
```
如何在局域网中进行部署，让其他用户也可以访问
```

得到的AI回答为：
```
要在局域网中部署课堂测试系统，让其他用户可以访问，您可以按照以下步骤操作：

## 📡 1. 修改服务器监听地址
默认情况下，Node.js服务器只监听本地地址（127.0.0.1），需要修改为监听所有网络接口（0.0.0.0），这样局域网内的其他设备才能访问。
...
app.listen(PORT, () => {
  ...
  localhost:${PORT}`);
  console.log(`默认教师账号: admin / 
  admin123`);
  console.log(`数据存储在: ${dataFilePath}
  `);
});
...
## 🔒 2. 配置防火墙
确保Windows防火墙允许3000端口的入站连接：

### 步骤：
1. 打开 控制面板 → 系统和安全 → Windows Defender 防火墙
2. 点击 高级设置
3. 在左侧面板选择 入站规则 ，然后点击右侧 新建规则
4. 选择 端口 → 下一步
5. 选择 TCP ，输入 特定本地端口 ：3000 → 下一步
6. 选择 允许连接 → 下一步
7. 保持默认设置（域、专用、公用）→ 下一步
8. 输入名称： 课堂测试系统 (3000) → 完成

## 📍 3. 查找服务器的局域网IP地址
其他用户需要通过您的局域网IP地址访问系统：

### 方法：
1. 在服务器电脑上打开命令提示符（Win+R → 输入 cmd → 回车）
2. 运行命令： ipconfig
...
## 📱 5. 多设备访问测试
### 测试步骤：
1. 启动服务器：运行 start.bat 或 node server.js
2. 在服务器电脑上访问： http://localhost:3000 （确认本地可访问）
3. 在局域网内的其他电脑/手机上访问： http://[服务器IP]:3000
4. 尝试学生登录和教师登录，确认功能正常
## 🛠️ 6. 注意事项
### 网络环境要求：
- 所有设备必须在同一局域网内（连接同一个路由器/交换机）
- 路由器设置中未启用AP隔离功能
- 网络防火墙未阻止3000端口
### 性能考虑：
- 如果同时有多个学生答题，建议服务器电脑配置足够的内存
- 避免在网络带宽有限的环境下进行大规模测试
## 📖 部署示例
假设您的服务器IP是 192.168.1.100 ，端口是 3000 ：
...
可以看到，AI自动执行了“ipconfig”命令来查看本机的ip地址，并对服务端代码 server.js 进行了修改，然后启动了服务。（windows系统防火墙相关设置用户可以根据AI给出的操作步骤自行完成设置）
![输入图片说明](/imgs/2026-02-01/7TdrLVbiEUML9wKv.png)

最后，我们在浏览器地址栏输入局域网访问网址“http://192.168.0.106:3000”，即可进入课堂测试系统。在局域网中使用另一台计算机通过网址“http://192.168.0.106:3000”也可以访问课堂测试系统，表明在局域网中部署成功。保持命令终端开启状态，则学生可以持续通过局域网中其他计算机访问我们的课堂测试系统。
![输入图片说明](/imgs/2026-02-01/ZbU7MnRNz848wE3q.png)
