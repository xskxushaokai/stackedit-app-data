...
7      for(int i = 1; i <= n; i++)
8          for(int j = 0; j <= C; j++){
9              if(c[i] > j) dp[i][j] = dp[i-1][j];  // 第 i 个物品比背包还大，装不下
...
3      if(i == 0) return 0;
4      int res;
5      if(c[i] > j) res = solve(i-1, j);    // 第 i 个物品比背包还大，装不下
...
DP 的状态方程常常是二维和二维以上，占用了太多空间。例如，**例4. 0/1背包** 的代码使用了二维矩阵 `int dp[N][C]`，设 $N=10^3, C=10^4$，都不算大，但 int 型占据 4B，矩阵需要的空间为 $4 \times 10^3 \times 10^4 \approx 40\text{MB}$，已经超过一般竞赛题的空间限制。 

用滚动数组可以大大减少空间。它能把二维状态方程 $O(n^2)$ 的空间复杂度优化到一维的 $O(n)$，更高维的数组也可以优化后减少一维。

从状态转移方程 $dp[i][j] = \max(dp[i-1][j], dp[i-1][j-c[i]] + w[i])$ 可以看出，$dp[i][j]$ 只与 $dp[i-1][]$ 有关，和前面的 $dp[i-2][], dp[i-3][], \dots$ 都没有关系。从前面的图表也可以看出，每行是通过上面一行算出来的，与更前面的行没有关系。那些用过的已经无用的 $dp[i-2][], dp[i-3][], \dots$ 多余了，那么干脆就复用这些空间，用新的一行覆盖已经无用的一行（滚动），只需要两行就够了。 
...
**举个例子：有4个物品，其体积分别为$\{2,3,6,5\}$，价值分别为$\{6,3,5,4\}$，背包总容量为9.**

(1) **$j$ 从小到大循环是错误的。** 例如，$i=2$ 时，下图  左侧的 $dp[5]$，经计算得到 $dp[5]=9$，把 $dp[5]$ 更新为 9。继续计算，当计算 $dp[8]$ 时，得 $dp[8]=dp[5]'+3 = 9+3=12$，这个答案是错的。错误的产生是由滚动数组重复使用同一个空间引起的。

$i=2$（体积 $c[2]=3$，价值 $w[2]=3$）：
![输入图片说明](/imgs/2026-01-14/aE2zazfbOgiJC9sX.png)

-   **错误点**：计算 $dp[5]$ 时，用到了旧的 $dp[2]$，更新为 9。但计算 $dp[8]$ 时，公式是 $max(dp[8]', dp[5] + w[2])$。此时的 **$dp[5]$ 已经是这一轮刚更新过的“新值” (9)**，导致 $9 + 3 = 12$。


(2) **$j$ 从大到小循环是对的。** 例如，$i=2$ 时，首先计算最后的 $dp[9]=9$，它不影响前面状态的计算，如下图所示。 

$i=2$（$c[2]=3, w[2]=3$）：
![输入图片说明](/imgs/2026-01-14/x7SAJTX6fdfX0Sbq.png)

-   **正确点：** 我们先算 $dp[9]$，它依赖的是 $dp[6]'$。因为是从后往前算，此时 **$dp[6]'$ 还是上一轮的旧值**，还没有被更新。这样就保证了每一件物品在这一轮里只会被考虑放入一次，符合 0/1 背包的定义。
