...
此时我们的问题就转化为：**给定一个距离dis，如何验证dis作为任意两间牛舍的最小距离时，n个位置足够分配m间牛舍？** 当任意给定一个dis值我们都可以进行验证后，就可以使用**枚举**所有可能的dis值的方法找到最大的dis。

很简单，定义一个check函数来验证两间牛舍的距离在大于等于dis的情况下，能否分配m间牛舍。我们只需要从第一个位置开始分配牛舍，下一间牛舍到上一间牛舍的距离必须大于等于dis，将符合条件的位置处分配牛舍，最后计算总共可分配的牛舍数量cnt，并将cnt与牛的数量m进行比较。代码如下：
```c++
bool check(int dis){
    int cnt = 1, place = 0;
    for(int i=1;i<n;i++){
...
接下来，我们仔细思考上面```check```函数中 ```cnt``` 和```dis```的关系，会发现，```cnt```的值随着```dis```的增大而减小，单调递减，```dis```的取值范围是0到$max(x_i - x_j)$，所以考虑使用二分查找来优化暴力枚举。那么二分查找的边界怎么确定呢？也就是dis的取值范围是多少呢？很简单，dis

<!--stackedit_data:
eyJoaXN0b3J5IjpbMTM3MDA2NDM1NCwtMjAwMDAzNDExMCwxMD
g5OTQ3MzA5LC0xMzExMjczMzkxLDEzMDA1MDAwMTksLTE3MDQ1
NjM0NTMsOTg2MzMyNjA2LC0xMjcyNjE3MTA3LC04MTgzMjI1OT
ksLTcxNDk3NjAxOSwxODQ3Mzg1ODQzLDY1MzA4MTA4MCw2OTAz
NjQyMDgsMjIwMzEzODc1LDM5NzIxNjU5NSwtMTk3MDE4ODk0Ny
w4NzEwNTIzNTUsNzIwMzY4NzYzLC0xOTU4OTAxNTUsLTE5NTQx
ODQ2MTNdfQ==
-->