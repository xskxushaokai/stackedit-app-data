...
int bin_search(int minl, int maxl){
    while(minl<=maxl){
        int mid = (minl+maxl)/2;
        int s = 0;
        for(int i=0;i<n;i++) s += a[i] / mid;    // 单调递增函数
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE3NDkwMzcyMjMsMTQxMTc2MDQzNCwxNT
UxODk1NDQsLTE0NDI4MzEwODMsLTg5NDEyMjU4MywtMzYxOTY4
MTI5XX0=
-->