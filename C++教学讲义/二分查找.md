...
接下来，我们仔细思考上面```check```函数中 ```cnt``` 和```dis```的关系，会发现，```cnt```的值随着```dis```的增大而减小，是一个离散的单调递减函数，```dis```的取值范围是 $0$ 到 $max(x_i - x_j)$，所以考虑在 $(0,max(x_i - x_j)]$ 区间内使用二分查找来找到满足条件的最大的 $dis$ 值 来优化暴力枚举：当 $dis$ 取到区间中点时，若牛舍数量不足$$
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUzNzQwMTM1NywtMTM1OTk1ODYzMywtMj
