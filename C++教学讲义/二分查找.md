...
很简单，定义一个```check```函数来验证两间牛舍的距离在大于等于dis的情况下，能否分配 ￥m间牛舍。我们只需要从第一个位置开始分配牛舍，下一间牛舍到上一间牛舍的距离必须大于等于dis，将符合条件的位置处分配牛舍，最后计算总共可分配的牛舍数量 $cnt$，并将 $cnt$ 与牛的数量 $m$ 进行比较。代码如下：
```c++
bool check(int dis){
    int cnt = 1, place = 0;
    for(int i=1;i<n;i++){
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTQyMTg3MDg2MiwtMTM1OTk1ODYzMywtMj
