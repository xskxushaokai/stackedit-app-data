...
此时我们的问题就转化为：给定一个距离dis，如何验证dis作为任意两间牛舍的最小距离时，n个位置足够分配m间牛舍？当任意给定一个dis我们都可以进行验证后，就可以使用枚举所有可能的dis值的方法找到最大的dis。

很简单，定义一个check函数来验证两间牛舍的距离在大于等于dis的情况下，能否分配m间牛舍。我们只需要从第一个位置开始分配牛舍，下一间牛舍到上一间牛舍的距离必须大于等于dis，计算总共可分配的牛舍数量cnt，并将cnt与牛的数量m进行比较。


<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTQyMTE0MDMsLTIwMDAwMzQxMTAsMT
