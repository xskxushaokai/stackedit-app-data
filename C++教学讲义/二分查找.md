...
接下来，我们仔细思考上面```check```函数中 ```cnt``` 和```dis```的关系，会发现，```cnt```的值随着```dis```的增大而减小，单调递减，```dis```的取值范围是 $0$ 到 $max(x_i - x_j)$，所以考虑在 $(0,max(x_i - x_j)]$ 区间内使用二分查找来优化暴力枚举
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEzNTk5NTg2MzMsLTIwMDAwMzQxMTAsMT
A4OTk0NzMwOSwtMTMxMTI3MzM5MSwxMzAwNTAwMDE5LC0xNzA0
NTYzNDUzLDk4NjMzMjYwNiwtMTI3MjYxNzEwNywtODE4MzIyNT
k5LC03MTQ5NzYwMTksMTg0NzM4NTg0Myw2NTMwODEwODAsNjkw
MzY0MjA4LDIyMDMxMzg3NSwzOTcyMTY1OTUsLTE5NzAxODg5ND
csODcxMDUyMzU1LDcyMDM2ODc2MywtMTk1ODkwMTU1LC0xOTU0
MTg0NjEzXX0=
-->