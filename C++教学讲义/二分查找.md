...
double bin_search(double left, double right){
    ...
    while(right-left>0.0001){
        double mid = (left+right)/2.0;  // mid值严格为[left,right]的中点，与整数二分的mid值是不同的
        double alpha = fun(w0,w,m,mid);
        // cout << mid << " mid " << alpha << endl;
        // cout << left << " left " << fun(w0,w,m,left) << endl;

        if (alpha==0) return mid;  // mid恰好为所求解
        else if (alpha*fun(w0,w,m,left)<=0) right = mid;  // fun(mid)与fun(left)异号，则根一定在[left,mid)之间，故后续查找将右端点移至mid处
        else left = mid;  // 否则根一定在(mid,right]之间，故后续查找将右端点移至mid处
    }
    return left;
}
...
```
<!--stackedit_data:
eyJoaXN0b3J5IjpbODcxMDUyMzU1LDcyMDM2ODc2MywtMTk1OD
kwMTU1LC0xOTU0MTg0NjEzLDE3NDY3MjI2NDcsMjA4NTk5Mzg5
LC0xNDU2OTY2MjE4LC01MzA5MTQxMzgsMTM1Mzg3Mzk1MCwxND
ExNzYwNDM0LDE1NTE4OTU0NCwtMTQ0MjgzMTA4MywtODk0MTIy
NTgzLC0zNjE5NjgxMjldfQ==
-->