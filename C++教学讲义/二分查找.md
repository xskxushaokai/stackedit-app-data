...
接下来，我们仔细思考上面```check```函数中 ```cnt``` 和```dis```的关系，会发现，```cnt```的值随着```dis```的增大而减小，是一个离散的单调递减函数，```dis```的取值范围是 $0$ 到 $max(x_i - x_j)$，所以考虑在 $(0,max(x_i - x_j)]$ 区间内使用二分查找来优化暴力枚举。
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk3ODgxMDkxMywtMTM1OTk1ODYzMywtMj
