...
接下来，我们仔细思考上面```check```函数中 ```cnt``` 和```dis```的关系，会发现，```cnt```的值随着```dis```的增大而减小，单调递减，所以考虑使用二分查找来优化暴力枚举。那么二分查找的边界怎么确定呢？也就是dis的取值范围是多少呢？很简单，dis

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk4NDY4NzQ4NSwtMjAwMDAzNDExMCwxMD
